...
public class EmissionManager {
            ...
            if (emissionTimer > 0) {
                ...
                if (emissionTimer == 2400) {
                    for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                        ...
                        playPersonalSound(player, StalkerMod.EMISSION_THUNDER, 5.0f, 0.8f);
                    }
                }
                ...
                if (emissionTimer < 1600 && emissionTimer > 600) {
                    if (server.getOverworld().random.nextInt(600) == 0) {
                        for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                            if (player.getServerWorld().getRegistryKey() != World.OVERWORLD) continue;
                            float randomPitch = 0.6f + (server.getOverworld().random.nextFloat() * 0.3f);
                            playPersonalSound(player, StalkerMod.EMISSION_THUNDER, 3.0f, randomPitch);
                        }
                    }
                }
                ...
                if (emissionTimer == 1200) {
                    for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                        if (player.getServerWorld().getRegistryKey() != World.OVERWORLD) continue;
                        playPersonalSound(player, StalkerMod.EMISSION_EARTHQUAKE, 2.0f, 1.0f);
                        playPersonalSound(player, StalkerMod.EMISSION_WIND, 1.5f, 1.0f);
                    }
                }
                ...
                if (emissionTimer < 1200 && emissionTimer > 230) {
                    if (server.getOverworld().random.nextInt(450) == 0) {
                        for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                            if (player.getServerWorld().getRegistryKey() != World.OVERWORLD) continue;
                            float randomWindVol = 2.0f + (server.getOverworld().random.nextFloat() * 2.0f);
                            float randomWindPitch = 0.7f + (server.getOverworld().random.nextFloat() * 0.5f);
                            playPersonalSound(player, StalkerMod.EMISSION_WIND, randomWindVol, randomWindPitch);
                        }
                    }
                }
                ...
                if (emissionTimer == 600) {
                    for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                        if (player.getServerWorld().getRegistryKey() != World.OVERWORLD) continue;
                        playPersonalSound(player, StalkerMod.EMISSION_EARTHQUAKE, 5.0f, 0.9f);
                    }
                }
                ...
                if (emissionTimer <= 2400 && emissionTimer % 20 == 0) {
                    for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                        ...
                        Box animalBox = player.getBoundingBox().expand(100.0);
                        ...
                        for (AnimalEntity animal : animals) {
                            ...
                            if (world.isSkyVisible(pos)) {
                                if (!animal.getNavigation().isFollowingPath()) {
                                    ...
                                    if (safePos != null) animal.getNavigation().startMovingTo(safePos.getX(), safePos.getY(), safePos.getZ(), 1.4);
                                }
                            } else {
                ...
                if (emissionTimer == 230) {
                    for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                        if (player.getServerWorld().getRegistryKey() != World.OVERWORLD) continue;
                        playPersonalSound(player, StalkerMod.EMISSION_EARTHQUAKE, 8.0f, 0.6f);
                    }
                }
                ...
                if (emissionTimer == 20) {
                    for (ServerPlayerEntity player : server.getPlayerManager().getPlayerList()) {
                        if (player.getServerWorld().getRegistryKey() != World.OVERWORLD) continue;
                        playPersonalSound(player, StalkerMod.EMISSION_BLOWOUT, 5.0f, 1.0f);
                    }
                }
    ...
    private static void playPersonalSound(ServerPlayerEntity player, net.minecraft.sound.SoundEvent sound, float volume, float pitch) {
